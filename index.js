...
const AUTOSERVICE_PRESET = 'islands#violetIcon';
const AUTOSERVICE_POINT_TYPE = 'avtoservis';
    ...
    fetch(DATA_URL)
        ...
        .then(obj => {
            ...
            obj.features.forEach(feature => {
                ...
                const isOverlay = isOverlayFeature(feature);
...
function prepareFeatureBalloon(feature, quantity, categories) {
    ...
    if (isAutoserviceFeature(feature)) {
        prepareAutoserviceBalloon(feature);
        return;
    }
...
function prepareAutoserviceBalloon(feature) {
    const properties = feature.properties || {};
    const title = getFeatureTitle(feature) || properties.name || 'Автосервис';
    const rows = [];

    if (properties.address) {
        rows.push(createBalloonRow('Адрес', properties.address));
    }

    if (properties.phone) {
        rows.push(createBalloonRow('Телефон', properties.phone));
    }

    if (properties.site) {
        const siteHref = normalizeExternalHref(properties.site);
        rows.push(createBalloonRow(
            'Сайт',
            siteHref ? createBalloonLink(siteHref, properties.site) : properties.site,
            Boolean(siteHref)
        ));
    }

    if (properties.url2gis) {
        rows.push(createBalloonRow(
            '2ГИС',
            createBalloonLink(properties.url2gis, 'Открыть'),
            true
        ));
    }
...
function createBalloonLink(url, text) {
...
function isAutoserviceFeature(feature) {
    const properties = feature.properties || {};

    return (
        properties.pointType === AUTOSERVICE_POINT_TYPE ||
        (feature.options && feature.options.preset === AUTOSERVICE_PRESET)
    );
}

function isOverlayFeature(feature) {
    return isGibddFeature(feature) || isAutoserviceFeature(feature);
}
...
function setupFilterUI(minQuantity, maxQuantity, objectManager, categories) {
    ...
    const autoserviceToggle = document.getElementById('autoservice-toggle');
    ...
    if (
        !dkToggleBtn || !categoryToggleBtn || !gibddToggle || !autoserviceToggle ||
    ...
    autoserviceToggle.classList.add('active');
    autoserviceToggle.setAttribute('aria-pressed', 'true');
    ...
    autoserviceToggle.addEventListener('click', () => {
        showAutoservice = !showAutoservice;
        autoserviceToggle.classList.toggle('active', showAutoservice);
        autoserviceToggle.setAttribute('aria-pressed', String(showAutoservice));
        applyFilter(currentMinQuantity, objectManager);
    });
...
function applyFilter(minQuantity, objectManager) {
    ...
    objectManager.setFilter(obj => {
        // ГИБДД и автосервисы управляются только своими кнопками.
        if (isGibddFeature(obj)) {
            return showGibdd;
        }

        if (isAutoserviceFeature(obj)) {
            return showAutoservice;
        }
